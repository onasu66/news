...
## Firestore に「キャッシュ」は必要？／Render 側キャッシュについて

- **Firestore = 永続化用**  
  Render のディスクはエフェメラルなので、再デプロイ・再起動で消えます。**解説（blocks・personas）や「どの記事が処理済みか」は Firestore に置かないと、再起動後にゼロからになり、API を大量に叩き直すことになります。** なので「キャッシュを Firestore に置く」のではなく「**永続ストアとして Firestore を使い、その読み取り回数を減らすために Render 側でメモリキャッシュを持つ**」形になっています。

- **Render 側のメモリキャッシュで読取を削減**  
  - **一覧**: `NewsAggregator._news_cache` に保持。`force_refresh` しない限り 2 回目以降は Firestore を読まない。  
  - **処理済み ID 一覧**: `get_cached_article_ids()` の結果を **5 分 TTL** でメモリキャッシュ。短時間の連続呼び出しは 1 回の Firestore 読取にまとまる。  
  - **記事詳細（解説）**: `get_cached(article_id)` の結果を **最大 400 件**までメモリキャッシュ。同じ記事を何度開いても、キャッシュヒット中は Firestore を読まない。

キャッシュを「Render 側に多めに持つ」ことで、Firestore の読み取り回数は減らせます（上記 TTL と件数はそのための設定です）。
...
- **get_cached_article_ids**  
  ...
  さらに **メモリで 5 分 TTL キャッシュ** し、短時間に何度呼ばれても 1 回の Firestore 読取にまとめる。

- **get_cached(解説)**  
  記事詳細を開くたび 1 読していた部分を、**Render 側メモリキャッシュ（最大 400 件）** で減らす。  
