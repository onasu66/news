...
def _do_seed_random_sync(count: int):
    """RSS取得→ランダムに count 件を選んで記事化（Firestore/SQLiteに保存）"""
    from app.services.rss_service import fetch_rss_news
    from app.services.article_processor import process_random_rss_articles
    from app.services.explanation_cache import get_cached_article_ids
    news = fetch_rss_news()
    if not news:
        return {"status": "error", "added": 0, "total": 0, "message": "RSSから記事を取得できませんでした。"}
    added = process_random_rss_articles(news, count=count)
    NewsAggregator.get_news(force_refresh=True)
    total = len(get_cached_article_ids())
    return {"status": "ok", "added": added, "total": total}


@router.get("/api/article/seed-random")
async def api_seed_random_articles(count: int = 3):
    """RSSからランダムに記事を選び、AI解説付きでFirestore/SQLiteに保存する。count=1〜10（既定3）"""
    if is_rss_and_ai_disabled():
    ...
    try:
        loop = asyncio.get_event_loop()
        result = await asyncio.wait_for(
            loop.run_in_executor(None, lambda: _do_seed_random_sync(count)),
            timeout=300.0,
        )
        return result
    except asyncio.TimeoutError:
        logger.warning("seed-random がタイムアウトしました")
        return {"status": "error", "added": 0, "total": 0, "message": "処理がタイムアウトしました（5分）。"}
    except Exception as e:
        logger.exception("seed-random でエラー")
        return {"status": "error", "added": 0, "total": 0, "message": str(e).strip() or "不明なエラー"}
