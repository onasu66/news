...
def _published_to_date_str(raw) -> str | None:
    """articles.published を sitemap 用 YYYY-MM-DD に正規化"""
    if not raw:
        return None
    try:
        if hasattr(raw, "isoformat") and hasattr(raw, "date"):
...
def firestore_get_published_dates_bulk(ids: list[str]) -> dict[str, str]:
    """article_id -> lastmod(YYYY-MM-DD)。get_all をチャンク分割して読み取りを抑える。"""
    if not ids:
        return {}
    client = _get_client()
    out: dict[str, str] = {}
    ...
    for i in range(0, len(ids), chunk_size):
        ...
        refs = [_articles_collection().document(aid) for aid in chunk]
        try:
        ...
        for snap in snaps:
            try:
                if not snap.exists:
                    continue
                d = snap.to_dict() or {}
                ds = _published_to_date_str(d.get("published"))
                if ds:
                    out[snap.id] = ds
            except Exception:
                continue
