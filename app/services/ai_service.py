...
PERSONAS = [
    ...
    {"id": 12, "name": "観測体オメガ", "emoji": "🔭", "role": "あなたは「観測体オメガ」である。ニュースを見た人間（一般読者・当事者・ネット世論など）がどう感じ・どう動くかを、心理学・群衆心理の観点から予測する観測者。恐怖・期待・安心・怒り・同一化・見て見ぬふりなど、心の反応パターンを短く述べる。善悪や政治的主張は書かない。文体は淡く無機質。ニュースの事実説明から入らず、人間側の反応予測から入る。箇条書きではなく自然な短文で。最後に、読者が自分の反応を一段メタで観察する一文を1つ入れる。", "type": "logic"},
...
def get_persona_opinion(
    ...
    if other_comments:
        ...
        if combined:
            avoid_note = (
                f"\n\n【他のキャラがすでに述べたコメント（重複・同調禁止）】\n{combined}\n"
                "↑上記と同じ言葉・フレーズ・結論の言い換えは禁止。同じ観点・比喩・動詞・名詞も使わない。"
                "制度・市場・生活・心理・倫理・技術・国際情勢など、他が触れていない軸だけから述べ、まったく違う角度のコメントにすること。"
            )
...
他キャラと話題が被るときは、争点をズラしてこの人格にしか書けない切り口だけに絞ること。{avoid_note}"""
    user_prompt = f"""【タイトル】{title}
