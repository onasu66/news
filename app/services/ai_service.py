...
■ 長さ（厳守）
・本文（textブロックの合計）は必ず1200字以上、目安は2500〜4500字。約3分で読める分量。
・1200字未満の出力は不可。入力が薄くても背景・仕組み・影響・具体例を補足して読み応えのある記事に膨らませる。
...
上記をもとに、ミドルマンとして日本語の記事を書いてください。本文 text 合計は必ず1200字以上。
...
def _generate_long_article_blocks(
    ...
    *,
    ...
    """1200字以上の text/explain blocks を生成。短い場合はフィードバック付きで再試行。"""
    from app.services.article_content_quality import is_generated_article_sufficient
                ...
                messages.append(
                    {
                        "role": "user",
                        "content": (
                            f"textブロック合計が{text_chars}字しかありません。最低1200字必要です。"
