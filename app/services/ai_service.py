...
def _normalize_text_explain_blocks(blocks: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Normalize repeated filler phrases in generated explain bubbles."""
    ...
    for block in blocks or []:
        ...
        if b.get("type") == "explain":
            content = re.sub(r"^(要するに|つまり|これって|実際)[、,。\s]+", "", content).strip()
            content = re.sub(r"^(.+?)っていうのは、?\s*要するに", r"\1とは", content).strip()
            content = re.sub(r"要するに[、,。\s]+", "", content).strip()
        b["content"] = content
        normalized.append(b)
    return normalized
...
# AIミドルマン：RSSを読み記事化。言い換えで水増しせず、情報密度を優先して本文を生成する。
...
■ 長さ
・情報密度を優先する。入力に新しい事実・数字・引用・比較材料が少ない場合は、無理に長くしない。
・短い入力を言い換えで水増ししない。背景・経緯を補足する場合も、記事から言える範囲に限る。
・検索で来た読者がそのページだけで要点を理解できるよう、記事内にある固有名詞・数字・過去の経緯・比較材料・関連サービス名を優先して使う。
...
・explainブロックで「要するに」「つまり」「これって」を口癖のように繰り返さない。毎回違う自然な言い出しにする。
・出力はJSON配列形式のみ。"""
...
■ 長さ（厳守）
・情報密度を優先する。入力に新しい事実・数字・引用・比較材料が少ない場合は、600〜900字程度でよい。
・入力に十分な材料がある場合（論文・技術解説・調査記事など）は 1,800〜2,600字を目標にする。
  ただし字数のために内容を薄めない。材料が尽きたらその時点で終える。
・同じ事実を言い換えて段落を増やすことは禁止。新しい情報が足せないなら短く終える。
・入力にない数字・コメント・比較データを作らない。
・検索で来た読者がそのページだけで満足できるよう、記事内にある固有名詞・数字・過去の経緯・他者や過去との比較・関連サービス名を優先して整理する。

■ 深さ（材料がある範囲で拾う。無い項目は書かなくてよい）
・具体的な手順・仕組み・アーキテクチャ（どう動くのか）
・数値結果と、その数値が何と比べてどうなのか
・従来手法・競合との違い（何が新しいのか）
・限界・未解決の点・再現性への注意
・読者が次に何を見ればよいか（原典・関連技術の名前）
これらは元記事の言い換えではなく、読者が原典を読む前に知りたい整理として書く。

■ explainブロック（吹き出し解説）のルール
・専門用語・業界用語・制度・仕組みが出てきたら、その直後に explain を入れる。
・1〜2文・80字以内（厳守）。「〇〇とは〜です」「ここで大事なのは〜です」のように、短く自然に説明する。
・「要するに」「つまり」「これって」を口癖のように繰り返さない。使うとしても記事全体で最大1回まで。
...
def explain_article_long_with_bubbles(
    ...
    model = model or settings.OPENAI_MODEL
    client = get_chat_client()
    user_prompt = f"""以下の記事を、友達に話すような喋り言葉で、情報密度の高い読み物にしてください。必ず日本語だけで出力してください。ところどころミドルマンの吹き出し（explain）も挟んでください。
...
■ やること
1) 記事本文を喋り言葉で書く。本文は新しい事実・数字・背景・過去の経緯・比較がある分だけ書く。検索した人が知りたい「何が起きたか」「なぜ重要か」「今後どうなるか」を自然に入れる。入力が薄い場合は600〜900字程度でよく、同じ事実の言い換えで水増ししない。入力に十分な材料がある場合は1,800〜2,600字を目標に、仕組み・数値結果と比較対象・従来との違い・限界・次に見るべき原典まで踏み込む。入力が英語でもすべて日本語で出力すること。
2) 適宜 explain ブロックでミドルマンが解説。記事の内容を補完するように、難しい部分を噛み砕いて教える。各 explain は1〜2文・80字以内（厳守）。長くなるなら explain を分けること。「要するに」「つまり」「これって」を繰り返さない。
3) blocks 配列のJSONのみ出力。すべて日本語で。"""
    ...
    if blocks:
        return _normalize_text_explain_blocks(blocks)
...
上記をもとに、ミドルマンとして日本語の記事を書いてください。情報密度を優先し、同じ事実の言い換えで水増ししないでください。検索で来た読者がそのページだけで要点を理解できるよう、記事内にある固有名詞・数字・過去の経緯・比較材料・関連サービス名を優先して使ってください。新しく言える事実・数字・背景・比較が少ない場合は短く終えてください。本文 text 合計の目安は{min_text}字ですが、薄い入力を無理に引き延ばさないでください。
...
def _parse_blocks_json(raw: str) -> list[dict[str, Any]]:
    ...
    if isinstance(data, list):
        return _normalize_text_explain_blocks(data) if _valid_text_explain_blocks(data) else []
    blocks = data.get("blocks", []) if isinstance(data, dict) else []
    return _normalize_text_explain_blocks(blocks) if _valid_text_explain_blocks(blocks) else []
...
def _generate_long_article_blocks(
    ...
    *,
    ...
    """情報密度と日本語比率を見ながら text/explain blocks を生成。不足時はフィードバック付きで再試行。"""
                    ...
                    messages.append(
                        {
                            "role": "user",
                            "content": (
                                f"textブロック合計は{text_chars}字です。必要な情報が十分にある場合だけ{min_text}字前後まで補強してください。"
                                "同じ事実の言い換えは禁止です。追加できる具体的な事実・数字・背景・比較がない場合は、短いままで構いません。"
                            ),
                        }
                    )
                continue
    ...
    if is_generated_blocks_quantity_sufficient(last_blocks) and blocks_mainly_japanese(last_blocks):
        return _normalize_text_explain_blocks(last_blocks)
    return None
...
def explain_article_inline_with_ai(
    ...
    min_text = min_generated_text_chars()
    user_prompt = f"""以下はRSSで取得した記事（タイトル＋本文）です。これを読んで、読者が情報密度高く読める記事にしてください。
...
■ やること
1. 上記の内容を把握する。
2. 記事本文（textブロック）を作る：内容が短い場合は、事実を変えずに背景・経緯・関連情報を補足する。検索した人が知りたい「何が起きたか」「なぜ重要か」「今後どうなるか」を自然に入れる。ただし同じ事実の言い換えで水増ししない。追加できる具体的な事実・数字・比較が少ない場合は短く終える。もともと長い場合は過度に要約せず、段落に分けて活かす。
3. 専門用語・固有名詞・略語・背景がある箇所の直後に、ミドルマンの解説（explain）を1つずつ挟む。解説は「人間が喋ってる風」の話し言葉で（です・ます調、親しみやすく）。平易な言葉だけを使い、背景や意味を説明しながら読み進められるようにする。「要するに」「つまり」「これって」を繰り返さない。
...
blocks配列のJSONのみ返す。"""
    ...
    try:
        ...
        if (
            ...
            return _normalize_text_explain_blocks(data)
        logger.warning(
...
def generate_editorial_take_fallback(
    ...
    try:
            ...
            messages=[
                {
                    "role": "system",
                    "content": (
                        "あなたはニュース解説メディアの編集部です。"
                        "記事の要約ではなく、読者が「へー、そういう見方があるのか」と感じる見立てを、"
                        "読者向けに自然な日本語で説明してください。"
                    ),
                },
                {
                    "role": "user",
        ...
        text = re.sub(r"^(編集部の見立て[:：]\s*)", "", text.strip())
        return _clean_editorial_take(text)
    except Exception as e:
        logger.warning("generate_editorial_take_fallback failed: %s", e)
        return ""
