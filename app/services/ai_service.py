...
■ 言語と長さ
・出力は必ず日本語で。英語の入力があっても日本語で記事を書く。
・本文（textブロックの合計）は読者が約3分で読める分量（2500字〜4500字）を目指す。短い入力なら背景・経緯・関連情報を補足して膨らませる。
・事実は変えず、推測は「〜とみられる」等で示す。

■ ブロックの並べ方
・textブロック＝記事本文の段落。複数に分けてよい。続き物として一つの記事になるように。
・explainブロック＝ミドルマンが「吹き出しで話す」解説。話し言葉・ですます調・親しみやすく。
・explain では「難しい内容」を説明すること：専門用語・制度・仕組み・背景など、読者が分かりにくい部分を平易な言葉で説明する。過去に同じテーマや関連する出来事があれば「以前も〇〇がありました」「昨年同様の動きが……」のように短く触れる。見出しやラベルは使わず、自然な語り口で。
...
def explain_article_long_with_bubbles(
    ...
    model = model or settings.OPENAI_MODEL
    client = OpenAI(api_key=settings.OPENAI_API_KEY)
    user_prompt = f"""以下の記事を、日本語で約3分で読める読み物にし、ところどころミドルマンの吹き出し（explain）を挟んでください。
...
■ やること
1) 記事本文を日本語で、約3分で読める分量（合計2500〜4500字）の複数 text ブロックで書く。英語の入力があっても日本語で書く。短い入力なら背景・経緯を補足して膨らませる。
2) 適宜 explain ブロックでミドルマンが日本語で解説。難しい用語・制度・背景を平易に説明し、過去の関連事例があれば短く触れる。各 explain は1〜3文・2行前後に収め、一度に長い話はしない。
